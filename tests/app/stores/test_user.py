...
def test_increment_usage_successful(user_store, sample_user_remote):
    ...
    with patch.object(user_store._store, 'get_rows_by_matching_condition', return_value=[mock_user_data]) as mock_get, \
        patch.object(user_store._store, 'convert_row_to_dict', return_value=mock_user_data) as mock_convert, \
        patch.object(user_store, 'update', return_value=True) as mock_update:
        
        result = user_store.increment_usage(api_key='valid_api_key', usage_counter=5)
        assert result
        mock_get.assert_called_once_with(column_to_match="api_key", matching_value='valid_api_key')
...
def test_increment_usage_invalid_api_key(user_store):
    with patch.object(user_store._store, 'get_rows_by_matching_condition', return_value=[]):
        with pytest.raises(Exception) as excinfo:
            user_store.increment_usage(api_key='invalid_api_key', usage_counter=5)
        assert 'API call is made even though the API key is invalid' in str(excinfo.value)

def test_increment_usage_database_exception(user_store, sample_user_remote):
    with patch.object(user_store._store, 'get_rows_by_matching_condition', side_effect=Exception("DB Error")):
        with pytest.raises(Exception) as excinfo:
