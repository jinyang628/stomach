...
def mock_user_service(mocker):
    ...
    mocker.patch("app.controllers.user_controller.UserService", return_value=mock_service)
    return mock_service
...
def test_validate_api_key_successful(client_with_mock_user_controller, mock_user_service):
    mock_user_service.validate_api_key.return_value = True
    response = client_with_mock_user_controller.get("/api/api_keys/validate/test_api_key")
    assert response.status_code == 200
    assert response.json() == {"message": "API Key successfully validated"}

def test_validate_api_key_unsuccessful(client_with_mock_user_controller, mock_user_service):
    mock_user_service.validate_api_key.return_value = False
    response = client_with_mock_user_controller.get("/api/api_keys/validate/invalid_api_key")
    assert response.status_code == 401
    assert response.json() == {"message": "Invalid API Key"}

def test_validate_api_key_exception(client_with_mock_user_controller, mock_user_service):
...
def test_increment_usage_successful(mock_user_controller, mock_user_service):
    # Arrange
    ...
    # Act
    result = mock_user_controller.increment_usage(api_key, tasks)
...
def test_increment_usage_failure(mock_user_controller, mock_user_service):
    # Arrange
    api_key = "test_api_key"
    ...
    with pytest.raises(HTTPException):
        mock_user_controller.increment_usage(api_key, tasks)
    mock_user_service.increment_usage.assert_called_with(api_key=api_key, tasks=tasks)
