...
async def test_get_all(mock_request):
    ...
    async_mock_to_list = AsyncMock(side_effect=[mock_data_batch1, mock_data_batch2, []])  # Returns two batches and then an empty list to end the loop
    mock_request.app.database["Entries"].find.return_value.skip.return_value.limit.return_value.to_list = async_mock_to_list
    ...
    assert entries == mock_data_batch1 + mock_data_batch2  # Combine the batches for the expected result
