...
SQL_VALUE_TO_TYPED_VALUE_VALID_DATA = [
    ...
    ({"age": "30"}, "age", int, 30),
    ({"timestamp": "2022-03-01 15:00:00"}, "timestamp", datetime, datetime(2022, 3, 1, 15, 0)),
...
@pytest.mark.parametrize("input_dict, key, expected_type, expected_output", SQL_VALUE_TO_TYPED_VALUE_VALID_DATA)
...
SQL_VALUE_TO_TYPED_VALUE_INVALID_DATA = [
    ({"key_containing_invalid_value": {"shawn": "kok"}}, "key_containing_invalid_value", dict, None),  
    ({"some_key": "some_value"}, "some_key", int, None), 
]
@pytest.mark.parametrize("input_dict, key, expected_type, expected_output", SQL_VALUE_TO_TYPED_VALUE_INVALID_DATA)
def test_sql_value_to_typed_value_invalid_data(input_dict, key, expected_type, expected_output):
