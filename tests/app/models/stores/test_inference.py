...
INFERENCE_LOCAL_VALID_DATA = [
    (
        ...
        "Test question 1",
        "Test half completed code 1",
        "Test fully completed code 1",
        "Test language 1"
    ),
    ("entry_id_2", "Test conversation 2", None, None, None, None, None, None),
]


@pytest.mark.parametrize(
    "entry_id, conversation, summary, summary_chunk, question, half_completed_code, fully_completed_code, language", INFERENCE_LOCAL_VALID_DATA
)
def test_inference_local_valid(entry_id, conversation, summary, summary_chunk, question, half_completed_code, fully_completed_code, language):
    inference = Inference.local(entry_id, conversation, summary, summary_chunk, question, half_completed_code, fully_completed_code, language)
    ...
    assert inference.half_completed_code == half_completed_code
    assert inference.fully_completed_code == fully_completed_code
    assert inference.language == language


INFERENCE_LOCAL_INVALID_DATA = [
    (None, "Test conversation", None, None, None, None, None, None),  # Missing entry_id
    ("entry_id", None, None, None, None, None, None, None),  # Missing conversation
]


@pytest.mark.parametrize(
    "entry_id, conversation, summary, summary_chunk, question, half_completed_code, fully_completed_code, language", INFERENCE_LOCAL_INVALID_DATA
)
def test_inference_local_invalid(entry_id, conversation, summary, summary_chunk, question, half_completed_code, fully_completed_code, language):
    with pytest.raises(ValueError):  # Or the specific error your code raises
        Inference.local(entry_id, conversation, summary, summary_chunk, question, half_completed_code, fully_completed_code, language)
...
INFERENCE_REMOTE_VALID_DATA = [
    {
        ...
        "half_completed_code": "Test answer 1",
        "fully_completed_code": "Test language 1",
