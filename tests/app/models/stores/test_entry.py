...
from app.models.stores.entry import ENTRY_VERSION, Entry
...
ENTRY_LOCAL_CONSTRUCTOR_VALID_DATA = [
    ...
    (
        ...
        "https://example.com/2",
        None,
        "expected_generated_id_2",
    ),  # Generated entry_id
]


@pytest.mark.parametrize(
    "api_key, url, entry_id, expected_entry_id", ENTRY_LOCAL_CONSTRUCTOR_VALID_DATA
)
@mock.patch(
    "app.models.stores.entry.generate_identifier",
    return_value="expected_generated_id_2",
)
def test_entry_local(
    mock_generate_identifier, api_key, url, entry_id, expected_entry_id
):
...
@pytest.mark.parametrize(
    "api_key, url, entry_id, expected_exception", ENTRY_LOCAL_CONSTRUCTOR_INVALID_DATA
)
